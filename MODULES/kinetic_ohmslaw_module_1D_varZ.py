...
def get_Biermann_classic(grid,rho_vyx,Bz_vyx,ne,Te_vyx):
    ...
    #---- now take the curl?
    # --- get_grad assumes [x,y] indices
    print ' np.shape(BierE_x) = ', np.shape(BierE_x),'len ygrid = ', np.shape(grid['y_grid']),'len x = ', np.shape(grid['x_grid'])
    ...
    #dyBierE_x, dxBierE_x = cf.get_grad(grid['y_grid'],grid['x_grid'],BierE_x)
    #dyBierE_y, dxBierE_y = cf.get_grad(grid['y_grid'],grid['x_grid'],BierE_y)
    print ' np.shape(BierE_x) = ',np.shape(I1_x), np.shape(omega_yx), np.shape(grid['y_grid']), np.shape(grid['x_grid']), np.shape(BierE_x),np.shape(BierE_y), np.shape(x_grid_yx),np.shape(y_grid_yx)
...
def get_vN_from_path(path,fprefix,time):
    ...
    for ix in range(nx):
       for iy in range(ny):
            Z2ni_loc = Z2ni[iy,ix]
            v_nx[iy,ix],v_ny[iy,ix] = get_v_N_classical(v_grid,Z2ni_loc,ne[iy,ix],Te[iy,ix],Bz[iy,ix],dxT[iy,ix],dyT[iy,ix])
