...
def newfig_generic_3lineouts(width=1.0):
    fig = newfig_generic_2yscale(1.2,
                                    scale_width=1.0,scale_ratio=0.5)
    return fig
...
def sci_fmt_colorbar(fig,im, ax, cax = '',lab=[]):
        ...
        if cax == '':
            ...
            c2 = fig.colorbar(im, ax=ax,  # ticker.FuncFormatter(custom_fmt),
                              label=lab)
        else:
            c2 = fig.colorbar(im, cax=cax, ax=ax,
                              format=ticker.FuncFormatter(sci_fmt),
                              label=lab)
...
def custom_colorbar(fig,im, ax, cax = '',lab=[]):
    ...
    if cax=='':
        ...
        c2 = fig.colorbar(im, cax=cax, ax=ax,#ticker.FuncFormatter(custom_fmt),
                      label=lab)
...
def get_fig_3plots(**kwargs):
    fig = newfig_generic_2yscale(1.4,
