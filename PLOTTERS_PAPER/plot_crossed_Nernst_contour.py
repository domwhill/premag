...
def colourbar(im, ax, cax, lab):
    cbar = plt.colorbar(im,ax=ax,cax = cax, label = lab)
    return cbar
...
def custom_contour(ax,xgrid,data,lab, lim, **kwargs):
    ...
    im = ax.contour(
                treat_data(data),levels = [-std_data,0,std_data],colors=(colors),
...
class run_obj_list:
    ...
    def __init__(self, var_type, scale_length=1, bz_in=50.0, lambda_p=5, pert_amp='1p', **kwargs):
        ...
        for ib, var in enumerate(self.var_list):
            ...
            self.save_tag = self.save_tag + str(scale_length)

    def get_max_time(self):
        time_in_list = []
        for tag in self.path_list:
            t_list, tc_list = cf.get_t_list(self.path_list[tag], var='Te')
            time_in_list.append(int(t_list[-1]))

        self.time_in_list = time_in_list
        time_in = int(np.min(time_in_list))
        self.t_max_index = '%02i' % (time_in)
        self.t_max_col = tc_list[time_in]
...
class PlotContour():
    ...
    def load(self):
        ...
        biermann_c, biermann_k = kohb.get_kinetic_and_classicB_c(self.path, fpre(self.path), self.time)
    ...
    def plot(self, fig, ax):
        ...
        self.im = custom_im(ax, self.x_grid, self.data, self.lab, lim)
        custom_contour(ax, x_grid_b, self.data_c, self.lab, lim=lim, colors='b')
        custom_contour(ax, x_grid_b, self.data_k, self.lab, lim=lim)
...
def main():
    ...
    obj_list = run_obj_list(var_type = 'bz', scale_length=2, bz_in = bz_list, lambda_p=5, pert_amp = '1p')
    ...
    # plot contour
    cont1 = PlotContour(obj_list.run_objs[0], var='Bz')
    cont2 = PlotContour(obj_list.run_objs[1], var='Bz')
    cont1.run(fig, ax1)
    cont2.run(fig, ax2)
    cont2.im.set_clim(cont1.im.get_clim())
    colourbar(cont2.im, ax3, cax=ax3, lab =cont1.lab)
    clear_yax(ax2)


    save_name = '%s_%s_bcross.png' % (obj_list.save_tag, obj_list.run_objs[0].save_tag)
