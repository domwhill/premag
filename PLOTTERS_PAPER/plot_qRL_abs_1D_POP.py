...
path_0T = paths.get_path(scale_len,'0',lambda_p_mfp,dim=dim)#path_pre + 'r5_v40_Z_FEOS_trackncrit_1D_0T_2'
...
class norm_labels(object):
    def __init__(self,var_in, var_data = []):
        if var_in=='Bz':    
            ...
            multT = 2.0
            self.lab = r'$T_e$[\SI{%1.1f}{\kilo\electronvolt}]' % (onesigfig((multT**-1)*2.0*Te_ref*1e-3))
            self.factor = 1.0
...
pT50, = ax2.plot(x_grid,dict_50T_k[var2]*v2_labs.factor
                    ,c='k',linestyle='--')
pT400, = ax2.plot(x_grid,dict_400T_k[var2]*v2_labs.factor,
                    c='k',linestyle=':')
...
if var_lim_400:
    set_ylim_max_2data(ax,x_grid,
                        dict_400T_c[var1]*v1_labs.factor,dict_400T_k[var1]*v1_labs.factor,
                        y_mult = [1.0,1.0],xlim=[xmin,xmax])

else:
    set_ylim_max_2data(ax,x_grid,
                        dict_50T_c[var1]*v1_labs.factor,dict_50T_k[var1]*v1_labs.factor,
                        y_mult = [1.0,1.0],xlim=[xmin,xmax])
