...
time_list = ['03', '05', '07']
path_list = []
bz_in = 50.0    # selected magnetic field [Tesla]
...
save_name = '%sdeltaqy_phase_angle_%s_%s_%s' % (save_path, save_tag, re.sub(' ', '', var), time)
phase_tag = '_angle_'
...
def get_angle_array(grid, xdata, ydata):
    """
        phase_arr = get_phase_array(grid,data,axis=0)
    """

    xphase_arr = get_phase_array(grid, xdata)
    yphase_arr = get_phase_array(grid, ydata)
    angle = yphase_arr - xphase_arr

    return angle


def conv_phase(phase):
    return phase / (2.0 * np.pi)    #(phase - np.pi)/(2.0*np.pi)
...
for ipp in range(len(path_list)):
    ...
    for ic, var in enumerate(var_list):
        ...
        phase_arr_c = get_angle_array(y_grid, dict_c[varx], dict_c[vary])
        phase_arr_k = get_angle_array(y_grid, dict_k[varx], dict_k[vary])

        p1, = ax5.plot(x_grid * cd5.xstep_factor, conv_phase(phase_arr_k), c=color, linestyle='-')
        ax5.plot(x_grid * cd5.xstep_factor, conv_phase(phase_arr_c), c=color, linestyle='--')
        p_list.append(p1)
