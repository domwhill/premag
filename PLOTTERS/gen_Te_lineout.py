...
def plot_Te_xlineout(fig,ax,path_list,var_amp='Te',time='15',vert_lines='False',dict_list = []):
    ...
    for pp in range(len(path_list)):
        ...
        p1, = ax.plot(x_c_grid[cl_index:c_index],T_data[cl_index:c_index,yi]*norm_const,c='k',linestyle=lstyle,marker=mstyle,markersize=MSIZE,
                    markevery=3,label=final_lab)
        leg_list_x.append(p1)
        lab_list_x.append(final_lab)
        save_name = save_name + str(h_lab)
        if vert_lines:
            cfoil.plot_xvlines(ax,x_c_grid,[],lineout_list,
                                custom_cmap=color_lineout,
                                linestyle=lh_style,dashes=dashes)
            #self.color_lineout
        '''
...
#-----------------------------------------------------------------------
def plot_custom_xlineout_amp(fig,ax,path_list,var_amp='Te',time='15',vert_lines='False',style_list = ['-','--',':','-','--',':'],mstyle_list = [None,None,None,'x','^','o'],leg_dict=[]):
    '''
    ...
    for pp in range(len(path_list)):
        ...
        p1, = ax.semilogy(x_c_grid[cl_index:c_index],np.abs(data_amp*norm_const),c='k',
                    linestyle=lstyle,marker=mstyle,markersize=MSIZE,
                    markevery=3,label=final_lab)
...
def plot_custom_xlineout_amp_tevol(fig,ax,path_list,var_amp='Te',time_list=['15'],vert_lines='False',style_list = ['-','--',':','-','--',':'],mstyle_list = [None,None,None,'x','^','o'],leg_dict=[],axleg=[]):
    '''
        p2 = plot_custom_xlineout(fig,ax,path_list,var_amp='Te')
    '''
    # --- init stuff
    ...
    for pp in range(len(path_list)):
        ...
        for tt in range(len(time_list)):
            ...
            p1, = ax.semilogy(x_c_grid[cl_index:c_index],np.abs(data_amp*norm_const),c=c_list[tt],
                        linestyle=lstyle,marker=mstyle,markersize=MSIZE,
                        markevery=3,label=final_lab)
...
#-----------------------------------------------------------------------
def plot_custom_xlineout(fig,ax,path_list,var_amp='Te',time='15',vert_lines='False',lstyle='-',mstyle=None):
    '''
        p2 = plot_custom_xlineout(fig,ax,path_list,var_amp='Te')
    '''
    #SI_on = True
...
#-----------------------------------------------------------------------    
def plot_ylineout_custom(fig,axy,path_list,var_amp='Te',time='15',mstyle_list = [None,None,None,'x','^','o'],style_list = ['-','--',':','-','--',':'],leg_on=True, dict_list = []):
    # --- init stuff
    if len(dict_list)!=0:
        color_lineout = dict_list['color_lineout']
        lineout_list = dict_list['lineout_list']
