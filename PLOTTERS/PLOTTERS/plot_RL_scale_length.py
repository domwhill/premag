...
path1 = path_pre + 'chfoil_default5_heat11_2D1--cx1'
...
path_1pp = path_pre + 'chfoil_default5_heat11_2D1_1pp--cx1'
path_1pp_95lmfp = path_pre + 'chfoil_default5_heat11_2D1_1pp_95lmfp--cx1'
path_speck = path_pre + 'cd5_speck_tc5.0ps_d5.0um_v3'
...
if len(sys.argv)>1:
    ...
    save_path = './pics'
path_tag = cf.retrieve_path_tag(path1)
#-----------------------------------------------------------------------
log_on = True
'''
SI_on = True
...
def get_amp_2D(U_data):
    ...
    #print ' np.shape(U_data) = ', np.shape(U_data), np.shape(avgxy_U), np.shape(np.sum((U_data - avgxy_U)**2,axis=1))
    U_dev = np.abs(U_data - avgxy_U)
    return U_dev
  
def plot_2D(ax,data,label,middle=None,colormap='jet',limits=None,xlabel=None,ylabel=None,clabel=None,fmt='%0.1e'):
...
if __name__ == "__main__":
    ...
    if True:
        ...
        fig.subplots_adjust(left=0.08,right=0.95,hspace=0.61,top=0.92) # adjust bottom so we can fit a legend underneath
        ...
        for tt in [11]:
            ...
            te_data = dict_te['mat'][:x_lim,1:-1]
            te_amp = get_amp_2D(te_data)

            dict_kinetic = extract_q(path,time,x_lim)
            ...
            for key in it_list:
                ...
                if it_list[-1]=='y':
                    ...
                    mid = None
                plot_2D(ax,data,label,middle=mid,colormap='RdBu_r',limits=lims,xlabel=ylab,ylabel=xlab,clabel=' ')
            ...
            ax2 = fig.add_subplot(1,2,2)
            q_RL_y = dict_kinetic['q_RL_y']['data']
            q_SH_y = dict_kinetic['q_SH_y']['data']
            print ' shape q_RL_y = ', np.shape(q_RL_y), np.argsort(q_RL_y)
            data = (q_RL_y + q_SH_y)/te_amp
            data_num = np.abs((q_RL_y+q_SH_y)/te_amp)
            data_denom = np.abs((q_SH_y + q_RL_y)/te_amp)
            data_num = np.abs((q_RL_y/q_SH_y))
            data_denom = 1.0*data_num


            dx = x_grid_SI[x_lim] - x_grid_SI[x_lim-1]
            print 'dx = ', dx, ' um'
            L_scale = (data_num[1:,:] - data_num[:-1,:])/((data_denom[1:,:] + data_denom[:-1,:])*0.5)
            L_scale = np.where(np.abs(data_denom[1:,:] + data_denom[:-1,:])<=1e-16,0.0,L_scale)
            L_scale_zero = np.average(data_num[1:,:] - data_num[:-1,:],axis=1)/np.average((data_denom[1:,:] + data_denom[:-1,:])*0.5,axis=1)
            L_scale_zero = np.average(L_scale,axis=1)
            
            L_scale = L_scale[10:,:]
            print 'shape L_scale = ', np.shape(L_scale)
            print 'L_scale limits = ', np.max(np.abs(L_scale)), np.min(np.abs(L_scale)),'\n std = ', np.std(np.abs(L_scale)),'  mean= ', np.mean(np.abs(L_scale))
            mid = None               


            clabel = r'$L_{q_y}$'
            im = ax.imshow(np.abs(L_scale),aspect='auto',vmin=0.0,vmax=0.5,norm=None)
            cbar = plt.colorbar(im,ax=ax,aspect='auto',norm=None,label=clabel)
            ax2.plot(L_scale_zero)
            ...
            savename = savepath + 'L_scale_'+str(time) + '.pdf'
