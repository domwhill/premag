...
if __name__ == "__main__":
    ...
    if True:
        ...
        for tt in [11]:
            ...
            print 'dx = ', dx, ' um'
            L_scale = (data_num[1:, :] - data_num[:-1, :]) / (
                (data_denom[1:, :] + data_denom[:-1, :]) * 0.5)
            L_scale = np.where(
                np.abs(data_denom[1:, :] + data_denom[:-1, :]) <= 1e-16, 0.0, L_scale)
            L_scale_zero = np.average(data_num[1:, :] - data_num[:-1, :], axis=1) / np.average(
                (data_denom[1:, :] + data_denom[:-1, :]) * 0.5, axis=1)
            L_scale_zero = np.average(L_scale, axis=1)

            L_scale = L_scale[10:, :]
            print 'shape L_scale = ', np.shape(L_scale)
            print 'L_scale limits = ', np.max(np.abs(L_scale)), np.min(
                np.abs(L_scale)), '\n std = ', np.std(np.abs(L_scale)), '  mean= ', np.mean(
                    np.abs(L_scale))
            mid = None
