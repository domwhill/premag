...
#-------------------------------------------->
path_pre = '../PREMAG/'    # path to directory where runs are stored
...
#def get_ratio():
#--------------- 0T loading--->
dict_qc, dict_qk = q_mod.get_q_abs(path_0T,
...
#--------------- 50T loading--->
dict_qc, dict_qk = q_mod.get_q_abs(path_50T,
                                   time)    #get_q_ratio(path,time)#get_q_individ(path,time)
v_nx_k, v_ny_k, v_nx_c, v_ny_c = q_mod.get_Nernst_abs(path_50T, time)
x_grid = dict_qk['x_grid'] * xstep_factor

#--------> debug

print(np.shape(dict_qc['SH x']), np.shape(dict_qk['SH x']), np.shape(dict_qk['tot x']),
      np.shape(dict_qk['tot x']), np.shape(dict_qc['tot x']))
print(np.shape(dict_qc['SH x']), np.shape(dict_qk['SH x']), np.shape(dict_qk['tot x']),
      np.shape(dict_qk['tot x']), np.shape(dict_qc['tot x']))
print(np.shape(x_grid))
print('v_nx = ', np.shape(v_nx_k), np.shape(v_nx_k), np.shape(v_ny_k), np.shape(v_nx_c),
      np.shape(v_ny_c))
...
#--------------- 50T loading--->
dict_qc, dict_qk = q_mod.get_q_abs(path_400T,
                                   time)    #get_q_ratio(path,time)#get_q_individ(path,time)
...
#----> plotting
print(' shape ratio = ', np.shape(rat_50T), np.shape(dict_qc['SH x'][:, 0]),
      np.shape(dict_qk['SH x'][:, iy]))
