...
var1_list = ['SH x']
var = var1_list[0]
marker_list = [None,'x','o','>']
var_list = ['SH x','SH y','RL x','RL y','E x','E y','vN x','vN y']
lab_list = [r'$q_{\perp,x}$',r'$q_{\perp,y}$',r'$q_{RL,x}$',r'$q_{RL,y}$',r'$q_{E,x}$', r'$q_{E,y}$', r'$q_{E,x}$',r'$v_{N,x}$',r'$v_{N,y}$']

ylab= ''
for var in var1_list:
...
def repack(path,time):
    ...
    print('time = ', dict_qk['time'], '= ', float(dict_qk['time'])*tstep_factor, 'ps')
    dict_out_c = {}
    dict_out_k = {}
    for var in var_list:
        if var[0] != 'v':
    ...
    dict_out_k['jx'] = dict_qk['jx'][:,iy]
    dict_out_k['jy'] = dict_qk['jy'][:,iy]
...
for var in var1_list:
    iv = var1_list.index(var)
    p0, = ax.plot(x_grid,dict_0T_k[var],c='g',marker=marker_list[iv])
    p0, = ax.plot(x_grid,dict_0T_c[var],c='g',linestyle='-.',marker=marker_list[iv])
    
    p50, = ax.plot(x_grid,dict_50T_k[var],c='r',marker=marker_list[iv])
    p50, = ax.plot(x_grid,dict_50T_c[var],c='r',linestyle='--',marker=marker_list[iv])
    p400, = ax.plot(x_grid,dict_400T_k[var],c='b',marker=marker_list[iv])
    p400, = ax.plot(x_grid,dict_400T_c[var],c='b',linestyle='--',marker=marker_list[iv])
...
pT50, = ax2.plot(x_grid,dict_0T_k[var2],c='gray',linestyle='-.')
pT50, = ax2.plot(x_grid,dict_50T_k[var2],c='gray',linestyle='--')
pT400, = ax2.plot(x_grid,dict_400T_k[var2],c='gray',linestyle=':')
