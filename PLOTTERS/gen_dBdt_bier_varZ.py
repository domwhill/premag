...
def plot_dBdt_bier(fig,ax,cax,path,time):
    ...
    CSy = ax.contour(data_bier_k[::-1,cl_index:c_index],levels=levels,colors=('k'),extent=lims_im)
...
def plot_ylineout_dtBz_custom(fig,axy,path_list,var_amp='Te',time='15',mstyle_list = [None,None,None,'x','^','o'],style_list = ['-','--',':','-','--',':'],leg_on=True, dict_list = []):
    # --- init stuff
    if len(dict_list)!=0:
