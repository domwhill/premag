...
def get_vN_from_path(path,fprefix,time):
    ...
    dict_Bz = cf.load_dict_1D(path,fpre(path),'Bz',time)
    Bz = dict_Bz['mat']

    dict_jxX = cf.load_dict_1D(path,fpre(path),'jxX',time)
    jxX = dict_jxX['mat']
    ...
    dict_qxX = cf.load_dict_1D(path,fpre(path),'qxX',time)
    qxX = dict_qxX['mat']
    ...
    wt = dict_wt['mat']
    dict_ne = cf.load_dict_1D(path,fpre(path),'n',time)
    ne = dict_ne['mat']
    ...
    dict_te = cf.load_dict_1D(path,fpre(path),'Te',time)
    Te = dict_te['mat']
    ...
    prof_Z = dict_Z['mat']
    nv,ny,nx = np.shape(fo)
    Z2ni = np.transpose(cf.trim_array_1D(ne,nx,ny))*np.transpose(cf.trim_array_1D(prof_Z,nx,ny))

    Bz = np.transpose(cf.trim_array_1D(Bz,nx,ny))
    jx = np.transpose(cf.trim_array_1D(jx_c,nx,ny))
    jy = np.transpose(cf.trim_array_1D(jy_c,nx,ny))
    '''
    
    dict  = cf.load_data_all_1D(path,fprefix,time)
    ne = dict['ne']
    Te = dict['Te'] 
    qx = dict['qx']
    qy = dict['qy']
...
def get_Nernst_ratio(path,time,sep_var=False):
    ...
    time_col = time_tcol*tstep_factor
...
def get_q_individ(path,time):
    ...
    Note q_x & q_y kinetic come in the saved formats, maybe we should change this... to take 'dict['qx']' loaded from load_data_all_1D
    '''
    time_int = int(time)
    ...
    nx = dict['nx']
    dict_fo = cf.load_dict_1D(path,fpre(path),'fo',time)
    print('--- q_individ = y =  ', np.shape(y_grid),'x shape = ', np.shape(x_grid))
...
def get_alpha_perp_path(path,time):
    ...
    #T_data = np.transpose(cf.trim_array_1D(Te,nx,ny))
    #n_data = np.transpose(cf.trim_array_1D(ne,nx,ny))
    T_data = Te
    n_data = ne
    Bz_data = Bz
    #---------------------------------------
...
if __name__ == "__main__":
    ...
    if True:
        ...
        for tt in t_list:
            ...
            dict_temp = cf.load_dict_1D(path,path.split('/')[-1],'Te',time)
            ...
            print(' classical_bier = ', classic_biermann, kinetic_biermann)
            #--- plot kinetic components            
            print('1582) =shape = classic bier=  ', np.shape(classic_biermann),'kinetic bier = ', np.shape(kinetic_biermann), 'shape NERNST = ', np.shape(data_x2d_vN_k),np.shape(data_y2d_tot_k),np.shape(data_x2d_tot_k))
            ...
            data_bier_k = kinetic_biermann[xi_min:x_lim,:]
            data_bier_c = classic_biermann[xi_min:x_lim,:]
            print('shape kin bier, classic bier = ', np.shape(data_bier_k),np.shape(data_bier_c),np.shape(classic_biermann[xi_min:x_lim].flatten()))
                        
            marker_q = None
            marker_nernst = None

            print('shapes = ',np.shape(x_grid_SI[xi_min:x_lim]), np.shape(data_bier_c),np.shape(np.abs(data_x_tot)), xi_min,x_lim)
            if log_on:
                ...
                #---
                print(' SHAPE xgrid = ', np.shape(x_grid_SI[xi_min:x_lim]),np.shape(np.abs(data_bier_k)[:]), 'BIER_FACTR = ', BIER_FACTR)
                print('data_bier_k = ', data_bier_k)
                #np.save('bier_k.npy',data_bier_k)
                #*
                p1bk, = ax3.plot(x_grid_SI[xi_min:x_lim],np.abs(data_bier_k)*BIER_FACTR,c='g')

                p1bc, = ax3.plot(x_grid_SI[xi_min:x_lim],np.abs(data_bier_c)*BIER_FACTR,c='g',linestyle='--')
            ...
            if log_on:
                ...
                p1nc, = ax1.semilogy(x_grid_SI[xi_min:x_lim],np.abs(data_x_vN_imp),
                c=c_n,linestyle=lstyle_c,marker=marker_nernst)
                
                #ax2.semilogy(x_grid_SI[xi_min:x_lim],np.abs(data_y_vN),
                #c=c_n,linestyle=lstyle_c,marker=marker_nernst)
                ax2.semilogy(x_grid_SI[xi_min:x_lim],np.abs(data_y_vN_imp),
                c=c_n,linestyle=lstyle_c,marker=marker_nernst)
