...
if __name__ == "__main__":
    ...
    for tt in range(len(t_list)):
        ...
        for ix in range(1,len(T_data[0,:])-1):
           for iy in range(1,len(T_data[:,0])-1):
                ...
                q_SH_xp, q_RL_xp,q_E_xp,q_SH_yp,q_RL_yp,q_E_yp = get_q_SH(Z2ni,n_data[iy,ix],T_data[iy,ix],Bz_data[iy-1,ix-1],dxT[iy-1,ix-1],dyT[iy-1,ix-1],jx[iy-1,ix-1],jy[iy-1,ix-1])
    ...
    for tt in range(len(t_list)):
        c = next(color)
        ax[0].plot(x_grid[1:-1],q_xX[tt,:-1,ny/2]/q_x[tt,:,ny/2],c=c,label=r'\textit{$' + str(leg_list[tt]) + '$}')
    ...
    for tt in range(len(t_list)):
        c = next(color)    
        ax[1].plot(y_grid[1:-1],q_yY[tt,xi,:-1]/q_y[tt,xi,:],c=c,label=r'\textit{$' + str(leg_list[tt]) + '$}')
    ...
    for tt in range(len(t_list)):
        ...
        ax[0].plot(x_grid[1:-1], q_RL_x[tt,:,ny/2]/q_SH_x[tt,:,ny/2],c='r',label ='RL/SH')
