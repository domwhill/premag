...
lab_list = [r'$q_{\perp,x}$',r'$q_{\perp,y}$',r'$q_{RL,x}$',r'$q_{RL,y}$',r'$q_{E,x}$', r'$q_{E,y}$',r'$v_{N,x}$',r'$v_{N,y}$']
ylab = lab_list[var_list.index(var)]
print(' var = ', var, ylab,var_list.index(var))
save_path = '/Users/' + userid + '/Dropbox/York/UPDATE/PICS_POP/'
save_name = save_path + 'qRL_abs_1d.png'
#---> norm
multT = 2.0
multQ = 1e-18
qrl_norm = (n0*1e6)*m_e*(v_te**3)*multQ
q_unit = r'$\,[\SI{%1.1e}{W/m^2}]$' % (multQ**-1) # kg s^-3 => kg m^-1 s^-2   ###nn    
#q_unit = r'$[\si{W/m^2}]$'# kg s^-3 => kg m^-1 s^-2   ###nn    
...
p50, = ax.plot(x_grid,dict_50T_k[var]*qrl_norm,c='r')
p50, = ax.plot(x_grid,dict_50T_c[var]*qrl_norm,c='r',linestyle='--')

p400, = ax.plot(x_grid,dict_400T_k[var]*qrl_norm,c='b')
p400, = ax.plot(x_grid,dict_400T_c[var]*qrl_norm,c='b',linestyle='--')
...
ax.set_ylim(-0.15*qrl_norm,0.01*qrl_norm)#(-0.1,0.01) - Nernst
    
#ax.set_ylim(vmin,vmax)#(-0.1,0.01) - Nernst
