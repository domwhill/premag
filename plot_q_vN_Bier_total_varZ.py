...
path_post = 'r5_v40_Z_FEOS_MODNE_5y_matchedf0_in_50T_E'
...
if __name__ == "__main__":
    ...
    if True:
        ...
        for tt in t_list:
            ...
            print('1) =shape = ', np.shape(classic_biermann), np.shape(classic_biermann[xi_min:x_lim,yi]), 'shape NERNST = ', np.shape(data_x_vN))
            ...
            if log_on:
                ...
                #---
                print(' SHAPE xgrid = ', np.shape(x_grid_SI[xi_min:x_lim]),np.shape(np.abs(data_bier_k)))
                p1bk, = ax3.semilogy(x_grid_SI[xi_min:x_lim],np.abs(data_bier_k)*BIER_FACTR,c='g')
                p1bc, = ax3.semilogy(x_grid_SI[xi_min:x_lim],np.abs(data_bier_c)*BIER_FACTR,c='g',linestyle='--')
        ...
        map(lambda axy: axy.set_xlim([-5.0,30.0]),[ax1,ax2,ax3])
        ...
        savename = save_path + 'q+vN_comparison+extra+Bier'
